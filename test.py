...
def extract_feature(model,dataloaders):
    ...
    count = 0
    if opt.linear_num <= 0:
        if opt.use_swin or opt.use_swinv2 or opt.use_vit:
            opt.linear_num = 1024
        elif opt.use_swinv2large or opt.use_swinlarge or opt.use_swinv2large_res384: 
            opt.linear_num = 1536
        else:
            opt.linear_num = 2048
...
if opt.use_swin:
    ...
    model_structure = ft_net_vit(opt.nclasses, linear_num=opt.linear_num)
elif opt.use_swinlarge:
    model_structure = ft_net_swinlarge(opt.nclasses, linear_num=opt.linear_num, imgsize=(opt.resx if opt.resx > 0 else w))
elif opt.use_swinv2:
    model_structure = ft_net_swinv2(opt.nclasses, (h,w),  linear_num=opt.linear_num)
elif opt.use_swinv2large:
    model_structure = ft_net_swinv2large(opt.nclasses, (h,w),  linear_num=opt.linear_num)
elif opt.use_swinv2large_res384:
    model_structure = ft_net_swinv2large_384(opt.nclasses, (h,w),  linear_num=opt.linear_num)
else:
